...
class Subscription_model extends CI_Model{
    ...
    public function activepanael($ConditionArray = []){
        $Result = $this -> db -> select(['P.purchase', 'P.expiry_date', 'R.name as user_name', 'R.id as register_id', 'R.email_id', 'S.price', 'S.name as plan_name', 'P.id', 'P.is_active'])
                                -> from('purchase as P')
                                -> join('subscription as S', 'P.sub_id = S.id', 'inner')
                                -> join('register as R', 'R.purchase_id = P.id', 'inner')
