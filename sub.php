    ...
    public function active_plan(){
        if($this ->authchecking()){
         ...
         if($this -> session -> userdata('AyaUserRole') == 1){
          $data['SubscriptionList'] = $this->subscription_model->activepanael(['P.register_id >' => 1]);
         }else{
          $data['SubscriptionList'] = $this->subscription_model->activepanael(['P.register_id' => $this -> session -> userdata('AyaUserLoginId')]);   
         }
